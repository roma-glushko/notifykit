...
class EventCollector:
    def __init__(self, notifier: NotifierT):
    ...
    async def collect(self) -> "AsyncContextManager[EventCollector]":
        if self._notifier_task is not None:
            raise RuntimeError("EventCollector is already running.")

        self._notifier_task = asyncio.create_task(self._collect_events())
    ...
    async def _collect_events(self) -> None:
        async for event in self._notifier:
